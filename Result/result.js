...
function addStudent() {
  ...
  const rollNumber = document.getElementById("rollNumber").value; // Keep as string for validation
  const name = document.getElementById("studentName").value;

  // Validate required fields (class, roll number, and name)
  if (!studentClass || !rollNumber || !name) {
    alert(
      "Please fill in all the required fields: Class, Roll Number, and Name."
    );
    return; // Exit the function if validation fails
  }

  const subjects = [];
  let allSubjectsFilled = true; // Track if all subjects are filled
  ...
  document.querySelectorAll(".subject").forEach((subject) => {
    ...
    if (!subjectMarks) {
      // If any subject marks are empty
      allSubjectsFilled = false;
    }

    subjects.push({ name: subjectName, marks: parseInt(subjectMarks) });
  });
  ...
  const student = {
    studentClass,
    rollNumber: parseInt(rollNumber), // Now parse to integer after validation
    name,
    subjects,
    totalMarks,
...
function updateStudent() {
  ...
  document.querySelectorAll(".subject").forEach((subject) => {
    const subjectName = subject
      ...
      .textContent.replace(":", "")
      .trim();
    const subjectMarks = parseInt(
      subject.querySelector(".subject-marks").value
    );
